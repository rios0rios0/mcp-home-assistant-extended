...
async def call_tool(name: str, arguments: dict) -> Sequence[TextContent]:
    ...
    try:
        if name == "list_automations":
            ...
            automation_id = arguments["automation_id"]
            result = await ha_api_call("DELETE", f"/automation/{automation_id}")
            return [TextContent(
                type="text",
                text=json.dumps({
                    "status": "deleted",
            ...
            automation_id = arguments["automation_id"]
            result = await ha_api_call("POST", f"/automation/{automation_id}/trigger")
            return [TextContent(
                type="text",
                text=json.dumps({
                    "status": "triggered",
            ...
            result = await ha_api_call("PUT", f"/automation/{automation_id}", current)
            return [TextContent(
                type="text",
                text=json.dumps({
                    "status": "enabled",
